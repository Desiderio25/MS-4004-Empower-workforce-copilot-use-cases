...
5. Skip any welcome messages if they appear.

6. After you selected the forward arrow, the **Agent Builder** form appeared for your new agent. The **Agent Builder** chat pane appears on the left, and the agent details appear on the right. At the top of the form are a **Configure** tab and a **Preview** tab.
...
10. Review Copilot's response after updating the instructions. To verify the changes that Copilot made, on the **Configure** tab, scroll down through the **Instructions** field. Verify that Copilot added the new instructions that you requested.
...
13. Once Copilot responds that it updated the instructions, on the **Configure** tab, scroll through the **Instructions**. Note the new items that Copilot added.
...
15. In the **Knowledge** section, select **+ Add knowledge**, and then select **Attach cloud files** (cloud icon). In the **Pick items** window that appears, locate and select the six files that you downloaded in step 1 and stored on your OneDrive.

16. For **Suggested prompts**, you can have Copilot generate prompts for you, or you can manually create your own prompts. Let's try both methods.  
...
17. You now want to enter several of your own prompts. On the **Configure** tab, scroll down to the **Suggested prompts** section. You should see the three prompts that Copilot added to the agent.  
...
18. Test several of the suggested prompts (you submit custom prompts in the next task). Verify the agent is correctly pulling in data from the knowledge source documents. You perform more extensive usage of this agent in a later task.

19. Once you're satisfied with the results for the suggested prompts, select the **Create** button to create the agent.

20. Once the agent is created, a dialog box appears that indicates the agent was successfully created. In this dialog box, you can either start chatting with the agent or share it. Select the **Start chat** option.
